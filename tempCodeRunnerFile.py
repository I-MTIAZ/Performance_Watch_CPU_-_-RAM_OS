...
class ResourceMonitorApp:
    def __init__(self, root):
        ...
        self.label_cpu.grid(row=0, column=0, padx=5, pady=5, sticky="w")

        self.progressbar_cpu = ttk.Progressbar(root, orient="vertical", mode='determinate', length=200)
        self.progressbar_cpu.grid(row=0, column=1, padx=5, pady=5)

        self.label_ram = tk.Label(root, text="RAM Usage: ", font=label_font)
        self.label_ram.grid(row=0, column=2, padx=5, pady=5, sticky="w")

        self.progressbar_ram = ttk.Progressbar(root, orient="vertical", mode='determinate', length=200)
        self.progressbar_ram.grid(row=0, column=3, padx=5, pady=5)
        ...
        self.canvas.get_tk_widget().grid(row=1, column=0, columnspan=4, pady=10, sticky="nsew")  # Span across all columns
        ...
        # Add buttons
        self.cpu_button = tk.Button(root, text="Show CPU", command=self.show_cpu)
        self.cpu_button.grid(row=2, column=1, pady=5)

        self.ram_button = tk.Button(root, text="Show RAM", command=self.show_ram)
        self.ram_button.grid(row=2, column=2, pady=5)

        self.both_button = tk.Button(root, text="Show Both", command=self.show_both)
        self.both_button.grid(row=2, column=3, pady=5)
